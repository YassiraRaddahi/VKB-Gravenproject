  ...
  <v-container fluid class="pa-0 list-page-container"> 
    ...
    <v-container fluid class="pa-4">
      ...
      <v-row density="comfortable">
        <v-col
          v-for="grave in filteredGraves"
          :key="grave.grave_number"
          cols="12"
          sm="6"
          md="4"
          lg="3"
        ...
        >
          <v-card
            elevation="2"
            class="grave-card d-flex flex-column h-100"
            @click="goToGrave(grave.id)"
            style="cursor: pointer;"
          >
            <v-img
              :src="grave.image_url"
              :key="grave.image_url + '-' + $route.fullPath"
              :alt="`Vooraanzicht van graf ${grave.grave_number} op ${cemetery?.name}`"
              height="220"
              cover
              class="grave-image"
            />
