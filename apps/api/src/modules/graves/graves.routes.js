module.exports = function (app, conn_db) {
  ...
  app.post("/api/cemeteries/:cemetery_id/graves", (req, res) => {
    ...
    conn_db.query(
      ...
      (checkErr, existing) => {
        ...
        const sql = `
                INSERT INTO graves
                    (cemetery_id, grave_number, type, sort, status, latitude, longitude, remarks)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `;
        ...
        conn_db.query(
          ...
          (err, result) => {
            ...
            res.status(201).json({ success: true, grave_id: result.insertId });
          }
        );
      }
    );
  ...
  app.put("/api/graves/:grave_id", (req, res) => {
    ...
    conn_db.query(
      sql,
      [grave_number, type, sort, status, remarks, id],
      (err) => {
        if (err) {
